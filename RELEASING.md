...
`github:mat3ra/ave#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/ave/releases/download/wip-<short-sha>/ave.tgz
```
...
```bash
npm run mat3ra:install -- ave wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/ave@<release-url>` with
